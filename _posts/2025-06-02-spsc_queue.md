...
memory we use bounded queue 99% of the time.Among the bounded queue there can be 2 types of 
...
In this post we will handle only Case 1 and give pointers how to handle case 2.There is not general solution for case 2.
It is usually context specific.
...
First let us start simple we build a queue using mutexs plain old way.
We have 2 pointers push and pop.Using mutexs we lock the entry into the push and pop functions.
Since we know that we have lock guard we just do the calculation in plain old way.
...
it will be removed.We need to handles cases when queue is full /empty.
This is a wait free queue means if we are not able to push or pop we return immediately.
