...
We have a function myrec which takes inp array as a parameter.In Dp problems the tough part

is to find the parameters which the recursive function will have.What changes over the recursive

calls, the index where we are at ..so let us have a paramter currindex.This index is the index

where we stand currently at start it is zero.Also we need some variable to  store how 

many ways we can reach at the last index so we have countofways.

Now we need to find out when and with what values will we call our rec function.

We just need to call the rec function num[i] times.Also note we need to provide return

conditions.This can be used to find the count of possible ways to go from
